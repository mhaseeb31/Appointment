...
with open(htmlfilepath) as htmlfile:
    ...
    #image_style = soup.find("form",{"id":captcha_selector_id}).div.div.captcha

    captcha_div = soup.find('div', id='content').find('captcha')

    style = captcha_div.find('div')['style']
                
    m = re.match("background:white url\('data:image/jpg;base64,(.+)'\).*", style)
    if(m):
